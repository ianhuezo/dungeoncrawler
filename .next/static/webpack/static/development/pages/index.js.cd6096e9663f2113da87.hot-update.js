  ...
  Object(_babel_runtime_corejs2_helpers_esm_createClass__WEBPACK_IMPORTED_MODULE_2__["default"])(Map, [{
    ...
    value: function searchRightColPosition(rowIdx, colSpan) {
      ...
      if (this.board[rowIdx][r].filled == false) {
        return [l, r];
          ...
          while (this.board[rowIdx][r].filled == true && counter < 3 && span > 3) {
    ...
    value: function searchTopBottomRowPosition(rowSpan, colIdx) {
      ...
      if (this.board[t][colIdx].filled == false && this.board[b][colIdx].filled == false) {
        return [t, b];
          ...
          while (this.board[t][colIdx].filled == false && this.board[b][colIdx].filled == false && counter < 3 && span > 3) {
            var topQuartile = Math.floor(span / 4 + t); //because bottom is lower in count...

            var bottomQuartile = Math.floor(b - span / 4);

            if (this.board[topQuartile][colIdx].filled == false && this.board[bottomQuartile][colIdx].filled == false) {
    ...
    value: function searchLeftColPosition(rowIdx, colSpan) {
      ...
      try {
        ...
        return null;
      } catch (e) {//console.log(rowIdx, l, r)
      }
    }
  }, {
    key: "placeLeft",
    ...
    value: function placeTop(pos, dir) {
      ...
      var roomSize = this.randomSize(5, 7);
      var rowSpan = [offset[0] - roomSize[0] + 1, offset[0] + 1];
      ...
      var rowSize = Math.abs(rowPosition[1] - rowPosition[0]);
      var colSize = Math.abs(colPosition[1] - colPosition[0]);
      console.log('row size after', rowSize);
      var room = new _room__WEBPACK_IMPORTED_MODULE_3__["Room"](rowSize, colSize);
      var halls = this.generateHalls(2);
    ...
    value: function searchTopRowPosition(colIndex, rowSpan) {
      var t = Math.abs(Math.ceil(rowSpan[0]));
      var b = Math.abs(Math.ceil(rowSpan[1]));
      ...
      var counter = 0; //if the index at t is not filled then you're fine, return 

      if (this.board[t][colIndex].filled == false) {
        return [t, b];
      } //else if its a bad index binary search for a good one unless there isn't one
      else {
          while (this.board[t][colIndex].filled == true && counter < 3 && span > 3) {
            var half = Math.ceil(t + span / 2);

            if (this.board[half][colIndex].filled == false) {
              return [half, b];
            } else {
              span = Math.abs(half - b); //console.log(span)
            }
    ...
    value: function searchBottomRowPosition(colIndex, rowSpan) {
      ...
      if (this.board[b][colIndex].filled == false) {
        return [t, b];
      } //else if its a bad index binary search for a good one unless there isn't one
      else {
          while (this.board[t][colIndex].filled == true && counter < 3) {
            var half = Math.floor(b - span / 4);
    ...
    value: function generateRooms(node) {
      ...
      for (var i = 0; i < removedHalls.length; i++) {
        ...
        this.board[row][col] = new _room__WEBPACK_IMPORTED_MODULE_3__["Node"]('-', [row, col]);
      }
...
//# sourceMappingURL=index.js.cd6096e9663f2113da87.hot-update.js.map